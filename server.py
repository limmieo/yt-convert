...
def process_video(brand):
    ...
    try:
        ...
        fc = (
            ...
            # bounce watermark (bottom-right, static)
            f"[wb]scale=iw*{sb}:ih*{sb},format=rgba,"
            f"colorchannelmixer=aa={ob}[bounce];"
            # static watermark (center-bottom, a bit lower)
            f"[ws]scale=iw*{ss}:ih*{ss},format=rgba,"
            f"colorchannelmixer=aa={os_}[static];"
            # top-left watermark (scrolling as before)
            f"[wt]scale=iw*{st}:ih*{st},format=rgba,"
            f"colorchannelmixer=aa={ot}[top];"
            # base video: mirror, desync, slight scale, centered crop, LUT, pad
            "[0:v]hflip,setpts=PTS+0.001/TB,"
            "scale=iw*0.98:ih*0.98,"
            "crop=iw-8:ih-8:(iw-8)/2:(ih-8)/2,"
            f"{lut_chain}"
            "pad=iw+150:ih+150:75:75:color=black[padded];"
            # overlay bounce (no shake)
            "[padded][bounce]overlay=x=main_w-w-30:y=main_h-h-60[b1];"
            # overlay static lower on screen
            "[b1][static]overlay=x=(main_w-w)/2:y=main_h-h-40[b2];"
            # overlay top scroller
